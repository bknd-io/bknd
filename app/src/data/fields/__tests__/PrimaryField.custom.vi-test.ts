...
describe('PrimaryField Custom ID Generation', () => {
  ...
  describe('Async ID Generation', () => {
    it('should generate UUID for uuid format using getNewValueAsync', async () => {
      const field = new PrimaryField('id', { format: 'uuid' });
    ...
    it('should return undefined for integer format using getNewValueAsync', async () => {
      const field = new PrimaryField('id', { format: 'integer' });
      
      const result = await field.getNewValueAsync();
      expect(result).toBeUndefined();
    });

    it('should generate custom ID using getNewValueAsync', async () => {
      const customHandler: CustomIdHandlerConfig = {
        type: 'function',
        handler: (entity: string, data?: any) => `${entity}-${data?.suffix || 'default'}`
      };
      ...
      const result = await field.getNewValueAsync('user', { suffix: 'test' });
      expect(result).toBe('user-test');
    });

    it('should throw error when entity name is missing for custom format', async () => {
      const customHandler: CustomIdHandlerConfig = {
        type: 'function',
        handler: () => 'custom-id'
      };
      ...
      await expect(field.getNewValueAsync()).rejects.toThrow(
        'Entity name is required for custom ID generation'
      );
    });

    it('should fallback to UUID when custom handler fails in getNewValueAsync', async () => {
      const consoleSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
      ...
      const result = await field.getNewValueAsync('user');
